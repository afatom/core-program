...
List*     ListCreate(void)
{
	...
	*/
    ...
    #ifdef HASH_TEST
    printf("Head adress %p  Head->m_prev adress %p\n",(void*)(&list->m_head),(void*)list->m_head.m_prev);
	printf("Tail adress %p  Tail->m_next adress %p\n",(void*)(&list->m_tail),(void*)list->m_tail.m_next);
	#endif
...
void ListDestroy(List** _pList, void (*_elementDestroy)(void* _item))
{
    ...
    if(NULL == *_pList || (*_pList)->m_magicNumber != LIST_VALID)
    {
        return;
    }
...
ListResult ListPushHead(List* _list, void* _item)
{
	...
	#ifdef HASH_TEST
    printf("Head adress         %p  Head->m_prev adress   %p\n",(void*)(&_list->m_head),(void*)_list->m_head.m_prev);
    printf("Tail adress         %p  Tail->m_next adress   %p\n",(void*)(&_list->m_tail),(void*)_list->m_tail.m_next);
	printf("Head->next adress   %p  tail->m_prev adress   %p\n",(void*)(_list->m_head.m_next),(void*)_list->m_tail.m_prev);
	printf("head.m_next->m_prev %p  tail.m_prev->m_next   %p\n",(void*)(_list->m_head.m_next->m_prev),(void*)_list->m_tail.m_prev->m_next);
