...
public:				 				//Member functions
	...
	int String_tCompare(const string_t& _str)const;
	...
	bool IsEqual(const string_t& _str)const; 		// ==
	bool Isdefferent(const string_t& _str)const;		// !=
	bool IsBigger(const string_t& _str)const;
	bool IsSmaller(const string_t& _str)const;
	bool IsBiggerOrEqual(const string_t& _str)const;
	bool IsSmallerOrEqual(const string_t& _str)const;
	bool IsContains(const string_t& _str)const; //ist calling obj contains str 
	static bool GetCaseSensFlag()const; //true=on
	static void SetCaseSens(bool state); //true=on false=off
	static void SetCapacity(size_t _capacity);
	static size_t GetCapacity(void)const;
