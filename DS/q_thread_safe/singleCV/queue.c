...
{
	#ifdef _DUAL_CV_
	if (pthread_cond_init(&_queue->m_cond2, NULL) != 0)
	{
		return FAIL;
	}
	#endif
	
	if (pthread_mutex_init(&_queue->m_mutex, NULL) != 0 || pthread_cond_init(&_queue->m_cond, NULL)!= 0 )
	{
...
{
	#ifdef _DUAL_CV_
	pthread_cond_destroy(&_queue->m_cond2);
	#endif
