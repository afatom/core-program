#ifndef __WT_QUEUE_SHP_T_H_
#define __WT_QUEUE_SHP_T_H_
...
class WtQueue_t{
    ...
    WtQueue_t();
    ~WtQueue_t();
    void push (const T& _item); //Inserts a new element at the end of the queue
...
template<typename T>
void  WtQueue_t<T>::push (const T& _item)
{
	pthread_mutex_lock(&m_mutex);
	try
	{
		m_queue.push(_item);
		cout<<"Thread was pushed succecc __WTQ MSG"<<endl;
	}
	catch (exception& e)
	{
		pthread_mutex_unlock(&m_mutex);  
